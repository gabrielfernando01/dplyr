...
wine <- read.csv("wine.csv", stringsAsFactors = FALSE, encoding = "UT-8")
wine <- wine[, -c(1, 3)]
...
ggplot(wine, aes(points, price)) + geom_point(color = "orange") + geom_smooth() + theme_bw()
...
ggplot(select_point, aes(x = reorder(country, points, median), y = points)) + 
  geom_boxplot(aes(fill = country), show.legend = FALSE) + xlab("country") + ylab("points") +
  ggtitle("Distribution of Top Ten Wine Producing Countries") + 
  theme(plot.title = element_text(hjust = 0.5)) + coord_flip() + theme_bw()
...
wine %>%
  ...
  ggplot(aes(reorder(variety, mediana), mediana)) +
	geom_col(aes(fill = variety), show.legend = FALSE) + xlab("Variety") + ylab("Median Points") +
	scale_x_discrete(labels = abbreviate) + coord_flip() + theme_bw()
